...
ssize_t ProcessFDSender::realWrite(const char* buff, size_t n, off_t offset){
    if(n != FD_SIZE){
        LOG_ERROR("n not is FD_SIZE, n:%ld, FD_SIZE:%d", n, FD_SIZE);
        errno = EBADF;
        return -1;
    }
...
ssize_t ProcessFDSender::realSendFD(FD recvFD, FD sendedFD){
    ...
    *((FD *)CMSG_DATA(unitptr)) = sendedFD; //CMSG_DATA()返回与cmsghdr 相关联的数据的第一个字节的指针
    ...
    LOG_DEBUG("sendmsg.wsize:%ld, errno:%d, recvFD:%d, sendedFD:%d", wsize, errno, recvFD, sendedFD);
    if(errno != EAGAIN){
        LOG_WARN("sendmsg error, wsize:%ld, sendedFD:%d, errno:%d, dataQueue.size():%d", wsize, sendedFD, errno, dataQueue.size());
    }
    return -1;
}
